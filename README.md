...
Every nudge is a choice between two things to do: the sensible one and the
ridiculous one. Making it a decision turns a notification people swipe away
into a five-second choice they act on — and either answer gets them off the
chair, which is the whole point.

The chaotic side follows hard rules, because a nudge that gets someone in
trouble is a bad nudge: nothing unsafe, nothing that damages anything, nothing
that makes a stranger the punchline, and nothing you couldn't do in a shared
...
per persona**, plus 50 chaos actions x 50 phrasings = 2,500 on the chaotic side.
Composing them beats listing them — writing 5,000 lines by hand would mean 5,000
chances to write a dull one.

The two sides run independent cursors. Sharing one would lock every task to the
same chaotic partner for good; separate cursors mean the pairing reshuffles as
the pools wrap at different rates.
...
```
src/
  data/tasks.ts        100 task actions + 50 chaos actions
